...
class BayesianOptimization(object):
    ...
    def __init__(self, f, pbounds, random_state=None, verbose=1):
        """
        :param f:
            Function to be maximized.需要优化的函数，也就是C=T*P

        :param pbounds:
            Dictionary with parameters names as keys and a tuple with minimum
            and maximum values. 变量的变化区间
        ...
        self.space = TargetSpace(f, pbounds, random_state)   #用于产生（x，y）
    ...
    def maximize(self,
        ...
        self.util = UtilityFunction(kind=acq, kappa=kappa, xi=xi)     #设置AC函数,可以为EI函数

        # Initialize x, y and find current y_max 进行初始化选点，可以默认选两个
        ...
        # Finding argmax of the acquisition function.  计算AC函数，选择下一个点。AC函数默认为UtilityFunction
